...
for weight in model.trainable_variables:
    if 'conv' in weight.name and 'kernel' in weight.name:
        ...
        group_norm.append(t1)
        group_norm.append(t2)
group_mean = tf.reduce_mean(tf.stack(group_norm))
print(group_mean)
for weight in model.trainable_variables:
    if 'conv' in weight.name and 'kernel' in weight.name:
        t1 = tf.reduce_sum(tf.abs(weight), axis=[0, 1, 2])
        ...
        t1 = tf.sqrt(tf.reduce_sum(t1))
        t2 = tf.sqrt(tf.reduce_sum(t2))
        if t1 > group_mean and t2 > group_mean:
            ugl_reg.append(t1 * 0.01 + t2 * 0.01)
            print('666')
        else:
            print('888')
            ul2_reg.append(0.01 * tf.math.reduce_sum(tf.math.square(weight)))

_prop_reg = tf.reduce_sum(tf.stack(ul2_reg)) + tf.reduce_sum(tf.stack(ugl_reg))
print(_prop_reg)
