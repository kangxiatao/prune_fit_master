...
def start_pruning(model, db_data, args, mode=None):
    ...
    _save_val_acc = model.evaluate(db_data, verbose=0)[1]
        ...
        if 'conv' in weight.name and 'kernel' in weight.name:  # conv2d
            ...
            if mode == 'auto':
                ...
                while _temp_threshold > 0.0000001:
                    ...
                    _acc = model.evaluate(db_data, verbose=0)[1]
    ...
    _accuracy = model.evaluate(db_data, verbose=0)[1]
    _prune_rate = _prune_rate / _layer_cnt

    return _accuracy, _prune_rate


def prior_pruning(model, db_data, args, mode=None):
    _accuracy, _prune_rate = 0.0, 0.0
    _layer_cnt = 0
    _prune_filter_n = 0
    _last_layer_prune_num = 0
